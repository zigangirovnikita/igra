...
export function V3Flow({ state, config: _config, dispatch, busy }: Props) {
  ...
  if (step === 'v3_story_budget') {
    return (
      <V3Screen image="beach-talk" title={`${name}, у нас в этом месяце все уже хорошо.`} busy={busy}
  ...
  if (step === 'v3_rules') {
    return (
      <V3Screen title="Правила запуска" busy={busy} button="Правила ясны!" onClick={() => dispatch('v3_next')}>
  ...
  if (step === 'v3_story_plan') {
    return (
      <V3Screen image="beach-talk" title="Подумайте о продукте и мечте" busy={busy}
  ...
  if (step === 'v3_product') {
    return (
      <V3Screen image="notebook" title={`Выберите, что будет продавать ${name}`} busy={busy}>
  ...
  if (step === 'v3_price') {
    ...
    return (
      <V3Screen image="notebook" title={`В какую стоимость ${name} будет продавать ${state.launchPlan.productName}?`} busy={busy}
  ...
  if (step === 'v3_dream') {
    ...
    return (
      <V3Screen image="notebook" title={`Что ${name} хочет купить, когда продажи пойдут?`} busy={busy}>
  ...
  if (step === 'v3_goal_summary') {
    return (
      <V3Screen image="summary" title="Цель запуска" busy={busy} button="Цель ясна" onClick={() => dispatch('v3_next')}>
  ...
  if (step === 'v3_reflection_intro') {
    return (
      <V3Screen title={`${name} начинает цикл запуска`} busy={busy} button="Понятно!" onClick={() => dispatch('v3_next')}>
  ...
  if (step === 'v3_prepare_category') {
    ...
    return (
      <V3Screen title={prepareArea ? AREA_TITLES[prepareArea] : 'Подготовиться'} busy={busy}>
        {!prepareArea ? (
          <div className="v3-stack">
  ...
  if (step === 'v3_advice_category') {
    return (
      <V3Screen title={adviceCategory ? adviceTitle(adviceCategory) : 'Посоветоваться'} busy={busy}>
        {!adviceCategory ? (
          <div className="v3-stack">
  ...
  if (step === 'v3_advice_result') {
    ...
    return (
      <V3Screen image="notebook" title="Совет получен" busy={busy} button="Вернуться к рефлексии" onClick={() => dispatch('v3_return_reflection')}>
        {advice ? (
  ...
  if (step === 'v3_rest') {
    return (
      <V3Screen title="Восстановить энергию" busy={busy}>
        <div className="v3-stack">
  ...
  if (step === 'v3_past_runs') {
    return (
      <V3Screen title="Прошлые попытки" busy={busy} button="Вернуться к рефлексии" onClick={() => dispatch('v3_return_reflection')}>
  ...
  if (step === 'v3_pre_action_summary') {
    if (!state.v3.lastPreparationSummary && (state.v3.plannedPreparations.length > 0 || state.v3.loopRestDays > 0)) {
      return (
        <V3Screen image="laptop" title="Готовим активный этап" busy={busy} button="Посчитать подготовку" onClick={() => dispatch('v3_begin_action_plan')}>
    ...
    if (summary) {
      return (
        <V3Screen image="laptop" title="Подготовка завершена" busy={busy} button="Да, начать активный этап" onClick={() => dispatch('v3_ack_pre_action_summary')}>
    ...
    return (
      <V3Screen image="laptop" title="Переходим к действиям" busy={busy} button="Выбрать инструменты" onClick={() => dispatch('v3_begin_action_plan')}>
  ...
  if (step === 'v3_action_select') {
    return (
      <V3Screen image="cocktails" title={`Решите, что ${name} будет делать в этот раз`} busy={busy}>
        <div className="v3-stack">
  ...
  if (step === 'v3_active_intro') {
    return (
      <V3Screen title="Сейчас начнется активный этап" busy={busy} button="Понятно!" onClick={() => dispatch('v3_next')}>
  ...
  return (
    <V3Screen title="Неизвестный экран v3" busy={busy}>
...
function V3Screen({
  ...
  return (
    <section className="v3-screen">
      {image && <PixelArtScene variant={image} />}
